  ...
  private async recoverCachedRecordings(): Promise<void> {
    ...
    for (const session of sessions) {
      ...
      try {
        ...
        // 会导致回放时 99% 的 Mutation 找不到父节点而冻屏。
        ...
        if (lastFullIdx >= 0) {
          ...
          // 带上紧邻在 FullSnapshot 之前的 Meta 事件（rrweb checkout 会成对产生）
          if (startIdx > 0 && allEvents[startIdx - 1].type === 4) {
            startIdx -= 1;
          }
          const candidate = allEvents.slice(startIdx);
          // 防御性校验：快照 id 空间应能覆盖后续 Mutation 的 parentId（计入增量新建的节点）。
          // 若不自洽（如缓存被部分淘汰出现断层），丢弃快照退化为"仅事件"，由业务靠前序段续播。
        ...
        for (let i = 0; i < recoveryEvents.length; i += RECOVERY_CHUNK_SIZE) {
          ...
          const chunk: RecordingChunk = {
            ...
            summary: {
              ...
              clickCount: 0, touchClickCount: 0, inputCount: 0, scrollCount: 0, selectionCount: 0,
