...
靠截图和模糊描述来复现用户反馈的 bug 非常痛苦。**sigillum-js** 录制完整的用户会话 —— 每一次点击、滚动、输入和页面跳转 —— 让你可以回放用户的真实操作。基于 [rrweb](https://github.com/rrweb-io/rrweb) 构建，完全自托管，数据不出你的服务器。

- **一行安装，零配置** — `npm install sigillum-js` 即可开始录制，无需额外安装任何依赖。
- **隐私优先** — 遮盖输入、屏蔽元素、脱敏敏感内容，全部内置。
- **防崩溃** — IndexedDB 缓存，页面崩溃后仍可恢复数据。
...
就这样。录制器自动捕获 DOM 变化、鼠标移动、滚动、输入和路由跳转。调用 `stop()` 后，数据通过你的 `onUpload` 回调上传。
